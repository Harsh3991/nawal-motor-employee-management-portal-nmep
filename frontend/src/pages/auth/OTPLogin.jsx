...
import { Mail, Loader, ArrowLeft, ArrowRight, ShieldCheck } from 'lucide-react';
...
const OTPLogin = () => {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      const response = await apiService.auth.verifyOTP({ email, otp: otpCode });
        ...
        <div className="relative z-10 p-12 text-white max-w-2xl">
          ...
          >
            ...
            <p className="text-lg text-slate-300 mb-8 leading-relaxed">
              Login quickly and securely with a One-Time Password sent directly to your email. No need to remember complex passwords.
            </p>
          </motion.div>
        </div>
      </div>
              ...
              >
                {otp.map((digit, index) => (
                  <input
                    ...
                    className="w-10 h-12 sm:w-14 sm:h-16 text-center text-2xl font-bold border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none bg-slate-50 focus:bg-white text-slate-900 shadow-sm"
                  />
                ))}
              </motion.div>
